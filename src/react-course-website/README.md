...
Try the application in your browser after parcel has built it. You will see we have access to the same level of functionality as the previous section, but it is far more robust and easy to work with. In particular, try to play around with the back and froward button of your browser, you will notice that the application doesn't refresh and that react-router takes over the navigation.
